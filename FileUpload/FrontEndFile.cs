...
namespace BlazorFileUpload
{
    public class FrontEndFile
    {
        ...
        public FrontEndFileStream CreateStream(IProgress<CopyProgress>? progressListener = null, double reportFrequency = 0.01, int maxMessageSize = 1024 * 31) => 
             Manager.CreateStream(this, progressListener, reportFrequency, maxMessageSize);
        ...
        public async Task<byte[]> GetAllContents(IProgress<CopyProgress>? progressReporter = null, double reportFrequency = 0.01)
        {
            ...
            using var stream = CreateStream(progressReporter, reportFrequency);
